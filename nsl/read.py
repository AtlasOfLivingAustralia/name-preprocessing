...
from nsl.schema import TaxonSchema, NameSchema, CommonNameSchema, RankMapSchema, TaxonomicStatusMapSchema, \
    OrthVarSchema, RelationshipSchema, SimplifiedRelationshipSchema, NKChecklistSchema, CLBChecklistSchema
...
def is_unknown_taxon(record: Record):
    return not is_accepted_taxon(record) and not is_synonym_taxon(record) and not is_misapplied_taxon(
        record) and not is_unplaced_taxon(record) and not is_excluded_taxon(record)
...
def fix_repeated_url(url: str) -> str:
    if url is None or not url.startswith('https://id.biodiversity.org.au/https://id.biodiversity.org.au'):
        return url
    return url[31:]
...
def check_misapplied_taxon(record: Record):
    if 'misapplied' in record.taxonomicStatus:
        return True
    else:
        return False


def canonical_name_check(record: Record):
    canonical_name = ""
    try:
        if record.canonicalName:
            ...
            sci_name_author = record.scientificNameAuthorship
            if sci_name_author:
                canonical_name = scientific_name.replace(sci_name_author, "").strip()
            else:
                canonical_name = scientific_name
    except:
        print("Exception for record:" + record.scientificName)
...
def reader() -> Orchestrator:
    ...
    nk_taxon_schema = NKChecklistSchema()
    ...
    with Orchestrator("nsl") as orchestrator:
        ...
        taxon_rank_lookup = LookupTransform.create("taxon_rank_lookup", scientific_taxon.output, rank_source.output,
                                                   'taxonRank', 'term', reject=True, record_unmatched=True,
                                                   lookup_map={'taxonRank': 'mappedTaxonRank',
                                                               'taxonRankLevel': 'taxonRankLevel'})
        nomenclatural_code_map = CsvSource.create("nomenclatural_code_map", nomenclatural_code_map_file, "ala",
                                                  nomenclatural_code_schema)
        establishment_means_map = CsvSource.create("establishment_means_map", establishment_means_map_file, "ala",
                                                   establishment_means_schema)
        ...
        taxon_coded = LookupTransform.create('taxon_coded', taxon_rank_lookup.output, nomenclatural_code_map.output,
                                             'kingdom', 'kingdom', record_unmatched=True,
                                             lookup_map={'nomenclaturalCode': 'kingdom_nomenclaturalCode',
        ...
        taxon_status_lookup = LookupTransform.create("taxon_status_lookup", taxon_coded.output, status_source.output,
                                                     'taxonomicStatus', 'Term', reject=True, record_unmatched=True,
                                                     lookup_map={'DwC': 'mappedTaxonomicStatus'},
                                                     lookup_include=['Accepted', 'Synonym', 'Misapplied', 'Unplaced',
                                                                     'Excluded'])
        # add canonical name if doesn't exist
        ...
        reference_taxon = MergeTransform.create("reference_taxon", accepted_taxon.output, synonym_taxon.output,
                                                misapplied_taxon.output, excluded_taxon.output)

        name_source = CsvSource.create("name_source", name_file, "excel", name_schema, no_errors=False)
        names_cleaned = MapTransform.create("names_cleaned", name_source.output, name_schema, {
        ...
        placed_name = FilterTransform.create('placed_name', names_cleaned.output, is_placed_name)
        vernacular_name = FilterTransform.create('vernacular_name', names_cleaned.output, is_vernacular_name)
        unused_name = FilterTransform.create('unused_name', names_cleaned.output, is_unplaced_name)

        accepted_name = LookupTransform.create('accepted_name', accepted_taxon.output, placed_name.output,
                                               'scientificNameID', 'scientificNameID', record_unmatched=True,
                                               lookup_prefix='name_', lookup_type=IndexType.FIRST)
        synonym_name = LookupTransform.create('synonym_name', synonym_taxon.output, placed_name.output,
                                              'scientificNameID', 'scientificNameID', record_unmatched=True,
                                              lookup_prefix='name_', lookup_type=IndexType.FIRST)
        misapplied_name = LookupTransform.create('misapplied_name', misapplied_taxon.output, placed_name.output,
                                                 'scientificNameID', 'scientificNameID', record_unmatched=True,
                                                 lookup_prefix='name_', lookup_type=IndexType.FIRST)
        unplaced_name = LookupTransform.create('unplaced_name', unplaced_taxon.output, placed_name.output,
                                               'scientificNameID', 'scientificNameID', record_unmatched=True,
                                               lookup_prefix='name_', lookup_type=IndexType.FIRST)
        excluded_name = LookupTransform.create('excluded_name', excluded_taxon.output, placed_name.output,
                                               'scientificNameID', 'scientificNameID', record_unmatched=True,
                                               lookup_prefix='name_', lookup_type=IndexType.FIRST)

        accepted_dwc = NslToDwcTaxonTransform.create("accepted_dwc", accepted_name.output, reference_taxon.output,
                                                     'taxonID', 'parentNameUsageID', 'accepted', 'parentNameUsageID')
        synonym_dwc = NslToDwcTaxonTransform.create("synonym_dwc", synonym_name.output, reference_taxon.output,
                                                    'taxonID', 'acceptedNameUsageID', 'synonym', 'acceptedNameUsageID')
        misapplied_dwc = NslToDwcTaxonTransform.create("misapplied_dwc", misapplied_name.output, reference_taxon.output,
                                                       'taxonID', 'acceptedNameUsageID', 'misapplied',
                                                       'acceptedNameUsageID', allow_unmatched=True)
        excluded_dwc = NslToDwcTaxonTransform.create("excluded_dwc", excluded_name.output, reference_taxon.output,
                                                     'taxonID', 'acceptedNameUsageID', 'excluded',
                                                     'acceptedNameUsageID', allow_unmatched=True)
        # taxon_dwc = MergeTransform.create("merge_dwc", accepted_dwc.output, synonym_dwc.output, misapplied_dwc.output,
        #                                   excluded_dwc.output)
        taxon_dwc = MergeTransform.create("merge_dwc", accepted_dwc.output, synonym_dwc.output, excluded_dwc.output)
        ...
        vernacular_source = CsvSource.create('vernacular_source', vernacular_name_file, "excel", common_name_schema,
                                             no_errors=False)
        vernacular_dwc = VernacularToDwcTransform.create('vernacular_dwc', vernacular_source.output, taxon_dwc.output,
                                                         'scientificNameID', 'scientific_name_id')
        vernacular_status = CsvSource.create("vernacular_status", vernacular_status_file, "ala",
                                             vernacular_status_schema)
        vernacular_dwc_filtered = DwcVernacularStatus.create("vernacular_dwc_filtered", vernacular_dwc.output,
                                                             vernacular_status.output)
        vernacular_dwc_output = CsvSink.create("vernacular_dwc_output", vernacular_dwc_filtered.output,
                                               "vernacularName.csv", "excel")

        dwc_identifier = DwcIdentifierGenerator.create('dwc_identifier', taxon_dwc.output, 'taxonID', 'taxonID',
                                                       DwcIdentifierTranslator.regex('^https://', 'http://',
                                                                                     title='Taxon', status='variant'),
                                                       fail_on_exception=True
                                                       )
        dwc_identifier_output = CsvSink.create("dwc_identifier_output", dwc_identifier.output, "identifier.csv",
                                               "excel", reduce=True)
        ...
        dwc_distribution_location_id = LookupTransform.create('distribution_location_id', dwc_distribution_base.output,
                                                              location_map.output, 'locality', 'locality',
                                                              lookup_include=['locationID'],
                                                              lookup_type=IndexType.FIRST, record_unmatched=True)
        dwc_distribution_location = LookupTransform.create('distribution_location', dwc_distribution_location_id.output,
                                                           location.output, 'locationID', 'locationID',
                                                           lookup_include=['locationID', 'locality'], overwrite=True,
                                                           record_unmatched=True)
        dwc_distribution = LookupTransform.create('distribution_establishment_means', dwc_distribution_location.output,
                                                  establishment_means_map.output, 'establishmentMeans', 'term',
                                                  lookup_include=['establishmentMeans'], overwrite=True,
                                                  record_unmatched=True)
        dwc_default_distribution = DwcDefaultDistribution.create('default_distribution', taxon_dwc.output,
                                                                 dwc_distribution.output, location.output)
        dwc_merged_distribution = MergeTransform.create('merged_distribution', dwc_default_distribution.output,
                                                        dwc_distribution.output)
        ...
        MetaFile.create('dwc_meta', taxon_dwc_output, vernacular_dwc_output, dwc_identifier_output,
        ...
        CsvSink.create("unplaced_taxon_output", unplaced_taxon.output, "unplaced_taxon.csv", "excel", True)
        CsvSink.create("unused_name_output", unused_name.output, "unused_name.csv", "excel", True)
    return orchestrator
...
def additional_reader() -> Orchestrator:
    ...
    relationship_file = "simplified_unplaced.csv"
    relationship_schema = SimplifiedRelationshipSchema()
    # Additional synonyms from APNI - uses same format as relationship file.
    ala_relationship_file = "ala_simplified_unplaced.csv"
    ...
    with Orchestrator("additional_nsl") as orchestrator:
        ...
        relationship_source = CsvSource.create("relationship_source", relationship_file, "excel", relationship_schema,
                                               no_errors=False)
        ala_relationship_source = CsvSource.create("ala_relationship_source", ala_relationship_file, "excel", relationship_schema,
                                               no_errors=False)
        ...
        dwc_ala_additional_synonyms = DwcAddAdditionalAPNISynonyms.create("dwc_ala_additional_synonyms",
                                                                      dwc_additional_synonyms.output,
                                                                      ala_relationship_source.output)

        dwc_unlinked_orthvar_filtered = FilterTransform.create("dwc_unlinked_orthvar_filtered",
                                                               dwc_ala_additional_synonyms.output,
        ...
        # added step to remove "misapplied" names from the taxon source so the names stay in the APNI taxon as potential legitimate names
        filtered_taxon_source = FilterTransform.create("filtered_taxon_source", taxon_source.output,
                                                       lambda r: not check_misapplied_taxon(r),
                                                       record_rejects=True)

        # Moved removing names in the taxon file till after the synthetic names are generated.
        unused_name = LookupTransform.create('unused_original_name', dwc_taxon.output, filtered_taxon_source.output,
                                             'scientificNameID', 'scientificNameID', lookup_type=IndexType.FIRST,
                                             reject=True, merge=False, record_unmatched=True)
        ...
        vernacular_dwc_output = CsvSink.create("vernacular_dwc_output",
                                               vernacular_dwc_filtered.output, "vernacularName.csv", "excel",
                                               reduce=True)

        MetaFile.create('additional_dwc_meta', dwc_output,
...
def nsk_reader() -> Orchestrator:
    ...
    vernacular_status_schema = VernacularStatusSchema()
    nk_taxon_schema = NKChecklistSchema()
    clb_taxon_schema = CLBChecklistSchema()
    with Orchestrator("nsl_nk") as orchestrator:
        taxon_source = CsvSource.create("taxon_source", taxon_file, "excel", clb_taxon_schema, no_errors=True,
                                        fail_on_exception=True)
        ...
        taxon_coded = LookupTransform.create('taxon_coded', taxon_rank_lookup.output, nomenclatural_code_map.output,
                                             ...
                                             lookup_map={'nomenclaturalCode': 'kingdom_nomenclaturalCode',
                                                         'taxonomicFlags': 'taxonomicFlags'})
        status_source = CsvSource.create("status_source", taxonomic_status_map_file, "ala", taxonomic_status_schema)
        #add canonical name field
        amended_source = MapTransform.create("amended_source", taxon_coded.output, TaxonSchema(),
                                              {
                                                  'canonicalName': (lambda r: canonical_name_check(r))
                                              }, auto=True)
        taxon_status_lookup = LookupTransform.create("taxon_status_lookup", amended_source.output, status_source.output,
                                                     'taxonomicStatus', 'Term', reject=True, record_unmatched=True,
                                                     lookup_map={'DwC': 'mappedTaxonomicStatus'},
                                                     lookup_include=['Accepted', 'Synonym', 'Misapplied', 'Unplaced',
                                                                     'Excluded'])
