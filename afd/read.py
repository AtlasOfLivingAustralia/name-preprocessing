...
def is_current_name(record: Record):
    return record.END_DATE is None and record.SCIENTIFIC_NAME is not None
...
def reader():
    ...
    with Orchestrator("afd") as orchestrator:
        ...
        name_filter = FilterTransform.create("current_name", name_source.output, is_current_name, record_rejects=True)
        ...
        referenced_name = LookupTransform.create("referenced_name", name_lookup.output, reference_format.output, 'NAME_ID', 'OBJECT_ID', lookup_type=IndexType.FIRST, record_unmatched=True)
        ...
        MetaFile.create('dwc_meta', dwc_taxon_output, dwc_vernacular_output, dwc_identifier_output, dwc_distribution_output)
