...
from processing.transform import LookupTransform, MapTransform, FilterTransform, ProjectTransform, \
    DenormaliseTransform, MergeTransform, DeduplicateTransform


def reader() -> Orchestrator:
    with Orchestrator("ala") as orchestrator:
        ...
        nomenclatural_code_map = CsvSource.create('nomenclatural_code_map', 'Nomenclatural_Code_Map.csv', 'ala',
        ...
        vernacular_list = FilterTransform.create("vernacular_list", species_list.output,
                                                 lambda r: r.vernacularName is not None and r.vernacularName != '-')
        ...
        dwc_vernacular_denormalised = DenormaliseTransform.delimiter("dwc_vernacular_denormalised",
                                                                     dwc_vernacular.output, 'vernacularName', ',')
        dwc_vernacular_identified = MapTransform.create("dwc_vernacular_identifier", dwc_vernacular_denormalised.output,
                                                        VernacularSchema(), {
                                                            'nameID': MapTransform.uuid()
                                                        }, auto=True)
        dwc_vernacular_output = CsvSink.create("dwc_vernacular_output", dwc_vernacular_identified.output,
                                               "vernacularName.csv", "excel", reduce=True)
...
def vernacular_list_reader() -> Orchestrator:
    with Orchestrator("ala_vernacular_list") as orchestrator:
        vernacular_list = VernacularListSource.create('vernacular_list', aliases={
            ...
            'raw Vernacular Name': 'vernacularName',
        ...
        names_unique = DeduplicateTransform.create('names_unique', name_transform.output,
                                                   ('scientificName', 'vernacularName', 'language'))
